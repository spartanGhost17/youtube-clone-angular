...
import { PlaylistsStateInterface } from '../../../shared/types/state/playlistsState.interface';
...
export class VideoComponent {
  ...
  @Input() videoURL: string =
    'http://localhost:8080/api/v1/video/watch/52532f11-0414-4982-8381-b9c6545d7212';
  ...
  constructor(
    ...
    private store: Store<{playlist: PlaylistsStateInterface}>,
  ...
  */
      ...
      next: (response: HttpResponseInterface<PlaylistInterface[]>) => {
        if(response) {
          ...
          if(!history) {
            ...
            this.playlistService.addVideo(videoItemForm).subscribe({});
          }
        }
      }
    });
  ...
  /**
   * get history if not set
   */
  getHistory() {
    this.store.select(selectPlaylists).subscribe({
      next: (playlists) => {
        const history = playlists.filter(pl => pl.name.toLowerCase().trim() === 'history')[0];
        if(!history) {
          this.store.dispatch(playlistActions.getHistory({pageSize: 10, offset: 0, userId: this.currentUser.id, name: 'history'}));
        } else {
  ...
  handleTimelineUpdate(event: any) {
    ...
    const previewImg = Math.max(
      1,
      Math.floor((percentage * this.videoDurationMillisec) / 10)
    );
  ...
  onVolumeChange(event: any) {
    ...
    } else if (this.video.nativeElement.volume >= 0.5) {
  ...
  toggleMute() {
    if (this.volumeLevel !== 'muted') {
      ...
      this.volumeLevel =
        this.currentVolume < 0.5 ? this.VOLUME_LOW : this.VOLUME_HIGH;
      this.video.nativeElement.muted = false;
      this.volumeSlider.nativeElement.value =
        this.currentVolume < 0.2 ? 0.3 : this.currentVolume;
  ...
  compareTimes(time1: string, time2: string) {
    ...
    const seconds1 =
      timeSplit1.length >= 1
        ? date1.setSeconds(timeSplit1[timeSplit1.length - 1])
        : 0;
    const minutes1 =
      timeSplit1.length >= 2
        ? date1.setMinutes(timeSplit1[timeSplit1.length - 2])
        : 0;
    const hours1 =
      timeSplit1.length >= 3
        ? date1.setHours(timeSplit1[timeSplit1.length - 3])
        : 0;

    const seconds2 =
      timeSplit2.length >= 1
        ? date2.setSeconds(timeSplit2[timeSplit2.length - 1])
        : 0;
    const minutes2 =
      timeSplit2.length >= 2
        ? date2.setMinutes(timeSplit2[timeSplit2.length - 2])
        : 0;
    const hours2 =
      timeSplit2.length >= 3
        ? date2.setHours(timeSplit2[timeSplit2.length - 3])
        : 0;
